...
```{r mean and median steps per interval sums}
...
```{r impute mean, message = FALSE, cache=TRUE}
...
```{r imputed steps per day histogram, message=F}
...
```{r chron library, weekend, group summary, message=FALSE}
...
```{r mean steps per interval weekend vs. weekday}
